...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, d_embed, n_head, max_seq_len=512, relative_position_embedding=True, causal_mask=False):
    ...
    def forward(self, x,c):
        ...
        # Apply linear layers.
        query = self.word_fc_layers[0](c)
        key = self.word_fc_layers[1](x)
        value = self.word_fc_layers[2](c)
        ...
        if self.relative_position_embedding and False: #TODO
...
def get_camsa_transformer_encoder(d_embed=512,
    ...
    attention_layer = MultiHeadAttentionLayer(d_embed, n_head, max_seq_len, relative_position_embedding, causal_mask)
